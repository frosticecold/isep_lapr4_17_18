...
public class Value implements Comparable<Value>, Serializable {
    ...
    public enum Type {
        ...
        TEXT,
        /**
         * Denotes a boolean value, i.YellowExtension. true or false
         */
        BOOLEAN,
        /**
        ...
        MATRIX,
        /**
         * Denotes an error, YellowExtension.g. a type mismatch
         */
        ERROR,
        /**
         * Denotes an form
    ...
    public Value(Form form){
        this.type = Type.FORM;
        this.value = form;
    }

    public Value(Money money){
        this.type = Type.MONETARY;
        this.value = money;
    ...
    public static Value parseValue(String value, Type... types) {
        // Uses default types
        if (types.length == 0) {
            types = new Type[]{Type.BOOLEAN, Type.DATE, Type.NUMERIC, Type.MONETARY};
        }
    ...
    /**
     * 07/06/2018 No longer able to parseDates, Domain having UI dependencies is
     * awful 1090657 - Raúl Correia Attempts to parse a date, time or date/time
    ...
    public static Value parseDateValue(String value) throws ParseException {
        ...
        // }
        // throw new ParseException(value, position.getErrorIndex());
        //return new Value("Unsupported");
    }

    public static Value parseMonetaryValue(String value) throws ParseException {
        String currencyStr = value.substring(value.length() -1);
        String amountStr = value.substring(0, value.length() -1);

        try {
            Currency currency = Currency.toCurrency(currencyStr);

            if (currency != null) {
                double val = Double.valueOf(amountStr);
                return new Value(new Money(val, currency));
            } else {
                throw new IllegalArgumentException("No currency found");
            }
        }catch (Exception e){
            throw new ParseException("Error while parsing monetary value: " +e.getMessage(), 0);
        }
    }
    ...
    public Form toForm() throws IllegalValueTypeException{
        if(type == Type.FORM){
            return (Form) value;
        }
        else throw new IllegalValueTypeException(this,Type.FORM);
    }
    ...
    public Money toMoney() throws  IllegalValueTypeException{
        ...
        if(type == Type.MONETARY){
            ...
            throw new IllegalValueTypeException(this, Type.MONETARY);
    }
